...
class Sql_structure
{
	...
	function get_selective_data($site_id, $current_id, $branch_entry_id, $mode, $show_depth, $max_depth, $status, $include, $exclude, $show_overview, $rename_overview, $show_expired, $show_future)
	{
		...
		$trailing_slash = $settings['add_trailing_slash'] === 'y';
	...
	function get_site_pages($cache_bust=false, $override_slash=false)
	{
		...
		if ($trailing_slash)
		{
			...
			{
				$site_pages['uris'][$key] = $this->EE->functions->remove_double_slashes($uri.'/');
			}
		}
	...
	function is_duplicate_listing_uri($entry_id, $uri, $parent_id)
	{
		...
		if ($query->num_rows > 0)
		{
			...
			$sql = "SELECT * FROM exp_structure_listings WHERE parent_id=20 AND uri REGEXP '^{$uri}.[0-9]'";
			$result = $this->EE->db->query($sql);
	...
	*/
	...
	function get_uri()
	{
		...
		$uri = preg_replace("/(\/P\d*)/", '', $this->EE->functions->remove_double_slashes('/'.$this->EE->uri->uri_string().$trailing_slash));
