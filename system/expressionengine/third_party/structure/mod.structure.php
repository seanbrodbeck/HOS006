...
class Structure
{
	...
	function nav()
	{
		...
		if ($start_from != '/')
		{
			$start_from = $this->EE->functions->remove_double_slashes('/'.html_entity_decode($start_from).'/');
	...
	{	
		...
		foreach ($result->result_array() as $entry)
		{
			...
			$crumbs[] = '<a href="' . $this->EE->functions->remove_double_slashes($home_link . $site_pages['uris'][$entry['entry_id']]) . '">' . $the_title . '</a>';
		}
