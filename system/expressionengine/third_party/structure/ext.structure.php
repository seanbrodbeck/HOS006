...
class Structure_ext {
	...
	function structure_ext($settings = '')
	{
		...
		$this->settings = $this->sql->get_settings();
	...
	{
		if ($cp_call === TRUE && isset($this->structure_settings['redirect_on_publish']) && $this->structure_settings['redirect_on_publish'] != 'n')
		{
			if ($this->structure_settings['redirect_on_publish'] == 'y')
				return BASE.AMP.'C=addons_modules'.AMP.'M=show_module_cp'.AMP.'module=structure';
			
			if ($this->structure_settings['redirect_on_publish'] == 'structure_only')
			{
	...
	{	
		...
		$trailing_slash = isset($this->settings['add_trailing_slash']) && $this->settings['add_trailing_slash'] === 'y' ? '/' : null;
		...
		$this->EE->config->_global_vars['structure:parent:url']        = $this->parent_id !== FALSE && $this->EE->config->_global_vars['structure:parent:uri'] !== FALSE ? $this->EE->functions->remove_double_slashes($this->site_pages['url'] . $this->EE->config->_global_vars['structure:parent:uri'])  : FALSE; // {parent:url}
		...
		$this->EE->config->_global_vars['structure:top:url']           = $this->segment_1 !== FALSE ? $this->EE->functions->remove_double_slashes($this->site_pages['url'].$this->EE->uri->segment(1).$trailing_slash)  : FALSE; // {top:url}		
	...
	function channel_module_create_pagination($ee_obj)
	{
		...
		else
		{
			...
			$new_basepath = $this->EE->functions->remove_double_slashes('/'.implode('/', $segment_array));
	...
	function core_template_route($uri_string)
	{
		...
		if (substr($last_segment,0,1) == 'P')
		{
			...
			$new_uri_string = $this->EE->functions->remove_double_slashes('/'.implode('/', $segment_array).$trailing_slash);
	...
	*/
	...
	function safecracker_submit_entry_end($obj)
	{
		...
		if (isset($obj->entry['entry_id']) && ($channel_type == 'page' || $channel_type == 'listing'))
		{
			...
			$fallback_template_id = isset($this->site_pages['templates'][$entry_id]) ? $this->site_pages['templates'][$entry_id] : $this->sql->get_default_template($channel_id);
			$fallback_parent_id = $this->sql->get_parent_id($entry_id) ? $this->sql->get_parent_id($entry_id) : 0;
			
			if (isset($obj->EE->api_sc_channel_entries->data['structure_uri']) && $obj->EE->api_sc_channel_entries->data['structure_uri'] != '')
			{
				$uri = url_title($obj->EE->api_sc_channel_entries->data['structure_uri']);
			}
			elseif ( ( ! isset($obj->EE->api_sc_channel_entries->data['structure_uri']) || $obj->EE->api_sc_channel_entries->data['structure_uri'] == '') && isset($this->site_pages['uris'][$entry_id]))
			{
				if ($channel_type == 'listing')
				{
					$listing_entry = $this->sql->get_listing_entry($entry_id);
					$uri = $listing_entry['uri'];
					$fallback_template_id = $listing_entry['template_id'];
				}
				else
				{
					$uri = $this->site_pages['uris'][$entry_id];
				}
			}
			else
			{
			...
			$template_id = isset($obj->EE->api_sc_channel_entries->data['structure_template_id']) && $obj->EE->api_sc_channel_entries->data['structure_template_id'] != "0" ? $obj->EE->api_sc_channel_entries->data['structure_template_id'] : $fallback_template_id;
			$parent_id   = isset($obj->EE->api_sc_channel_entries->data['structure_parent_id']) ? $obj->EE->api_sc_channel_entries->data['structure_parent_id'] : $fallback_parent_id;
			$parent_uri  = isset($this->site_pages['uris'][$parent_id]) ? $this->site_pages['uris'][$parent_id] : '/';
			$hidden      = isset($obj->EE->api_sc_channel_entries->data['structure_hidden']) && $obj->EE->api_sc_channel_entries->data['structure_hidden'] == 'y' ? TRUE : FALSE;
			$listing_id  = $this->sql->get_listing_channel_by_id($entry_id);

			// Now mash the parent and child uris together
			// $uri = $this->EE->functions->remove_double_slashes($parent_uri.'/'.$uri);
			$uri = $this->sql->create_page_uri($parent_uri, $uri);
			...
			{
				$entry_data['parent_id']   = $this->sql->get_listing_parent($channel_id);
				$entry_data['listing_cid'] = $this->sql->get_listing_channel($parent_id);
				$entry_data['parent_uri']  = $parent_uri;
	...
	{
		$url = array_key_exists($m[1], $this->site_pages['uris']) ? $this->EE->functions->remove_double_slashes($this->site_pages['url'] . $this->site_pages['uris'][$m[1]]) : NULL;
		return $url;
	}
