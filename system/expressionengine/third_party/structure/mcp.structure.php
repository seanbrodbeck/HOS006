...
class Structure_mcp
{
	...
	function index($message = FALSE)
	{
		...
		$settings_json = $this->EE->javascript->generate_json($settings_array, TRUE);
	...
	{
		$closed_ids = $this->EE->javascript->generate_json($this->EE->input->get_post('collapsed'), TRUE);
		...
		die($this->EE->javascript->generate_json($data, TRUE));
	}
	...
	function channel_settings_submit()
	{
		...
		{
			$this->EE->cp->set_variable('cp_page_title', $this->EE->lang->line('delete_channels'));
			$this->EE->cp->set_breadcrumb($this->base_url, $this->EE->lang->line('structure_module_name'));
